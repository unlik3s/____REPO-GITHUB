...
## Version : 1.0
## Description: Analyse crypto used by clients parsing IIS log
...
## Usage : Example 1 : .\Analyse_crypto_v1.0.ps1 -HTTPCipher $true -AgeLog "-24" -jobsNumber 5 -Output "c:\Scripts\Results"
################################################################################################################################################

param (
    [Parameter(Mandatory=$true)] [Bool]$HTTPCipher=$true,
    [Parameter(Mandatory=$true)] [string]$AgeLog=-1, #Specify age of IIS age to search in hours
    [Parameter(Mandatory=$true)] [string]$jobsNumber=5, #Specify max jobs to used to improve performance in large environment
    [bool] $SMTPReport=$false,
    [string] $Output="C:\SCRIPTS\Analyse-crypto\Results"
    )

#Variables
$servers = "SERVER1","SERVER2","SERVER3"
$UniqueValue = "c-ip"  #Specify value to sort unique object in the final report. Example : X-Forward-For (ideal or c-ip if IIS log contains original IP of the client)

#ReportAddress
$SMTPServer = "SERVER1"
$From = "noreply@yourdomain.fr"
$To = "youraddress@yourdomain.fr"
$Subject = "HTTPS Exchange cipher report"
#Css style
$css = @"
<style>
...
$SQLQueryHTTP = @"
SELECT date, time, s-computername, cs-username, c-ip, cs(User-Agent) as UserAgent,X-Forward-For, cs-uri-stem, crypt-cipher, crypt-hash, crypt-keyexchange, crypt-protocol
...
foreach ($csv in $ALLCSV){
    $ScriptBlock = {
    #Function to convert the number to readable
...
$dataToAnalyse = $dataToAnalyse | Sort-Object -Unique -Property $UniqueValue #X-Forward-For
...
#SMTP Report
if($SMTPReport){
$body = Get-Content "$Output\IIS-WeakProtocol-Result-HTML.html" -Raw
Send-MailMessage -SmtpServer $SMTPServer -From $From -To $To -Subject $Subject -Body $body -BodyAsHtml # other parameters
} 
